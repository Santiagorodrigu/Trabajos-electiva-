...
# Definir la rejilla de parámetros a sintonizar
grid <- expand.grid(k = c(1, 3, 5, 7, 9))

# Ajustar el modelo KNN con búsqueda de mejor k y validación cruzada
model <- train(
  x = X_train,
  y = y_train,
  method = "knn",
...
# Realizar predicciones en el conjunto de prueba con el mejor k
predictions <- predict(model, newdata = X_test)

# Evaluar el rendimiento del modelo con el mejor k
